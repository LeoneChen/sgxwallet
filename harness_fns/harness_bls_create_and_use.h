...
static void harness_bls_create_and_use(void) {
  ...
  // Get decryption share with valid "d1:d2:d3:d4" format public_decryption_value
  // SplitStringToFq splits by ':' and parses each token as decimal bigint
  char *pub_dec_val = (char *)calloc(320, sizeof(char));
  char *dec_share = (char *)calloc(320, sizeof(char));
  if (!pub_dec_val || !dec_share) return;
  // Generate 4 decimal numbers separated by ':'
  uint64_t d0 = g_fdp->ConsumeIntegral<uint32_t>();
  uint64_t d1 = g_fdp->ConsumeIntegral<uint32_t>();
  uint64_t d2 = g_fdp->ConsumeIntegral<uint32_t>();
  uint64_t d3 = g_fdp->ConsumeIntegral<uint32_t>();
  snprintf(pub_dec_val, 319, "%" PRIu64 ":%" PRIu64 ":%" PRIu64 ":%" PRIu64, d0, d1, d2, d3);
  errStatus = 0;
