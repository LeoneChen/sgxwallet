...
// Target: trustedCreateBlsKey with [in, count=6145] const char* s_shares
// Strategy: fill all 6145 bytes with no \0
// Hypothesis: if enclave calls strlen(s_shares), it reads past 6145 bytes

static void harness_create_bls_key_no_null(void) {
...
HARNESS_REGISTER(harness_create_bls_key_no_null, 50)
