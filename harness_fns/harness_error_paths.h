...
// DISABLED: redundant with 3 micro harnesses that each test its targets at lower cum:
// - harness_micro_bls_corrupt_key_sign_fixed (cum=1330) → BlsSignMessage with corrupt key
// - harness_micro_bls_pub_corrupt_key_fixed (cum=1380) → GetBlsPubKey with corrupt key
// - harness_micro_decryption_share_corrupt_key_fixed (cum=1430) → GetDecryptionShare with corrupt key
...
// HARNESS_REGISTER(harness_bls_sign_invalid_hash, 55)  // DISABLED: redundant with harness_micro_bls_sign_invalid_hash_fixed (cum=1280, zero consumption)
// DISABLED: redundant with harness_bad_hash_ecdsa (data-driven, same error path, more diverse)
