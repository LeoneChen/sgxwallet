...
AC_DEFUN([SGX_INIT],[
	...
	AC_SUBST(SGX_ENCLAVE_CFLAGS,
		["-fvisibility=hidden -fPIC -ffunction-sections -fdata-sections -fstack-protector -fsanitize=address -mllvm -asan-enclave-v -mllvm -asan-use-after-return=never -mllvm -asan-opt-globals=false -fsanitize-coverage=inline-8bit-counters,pc-table -fno-discard-value-names"])
	...
	AC_SUBST(SGX_ENCLAVE_LDADD,
		["-Wl,--whole-archive -lsgxsan_enclave -l\$(SGX_TRTS_LIB) -Wl,--no-whole-archive -Wl,--start-group \$(SGX_EXTRA_TLIBS) -lsgx_tstdc -lsgx_tcrypto -l\$(SGX_TSERVICE_LIB) -Wl,--end-group -Wl,-Bsymbolic -Wl,-eenclave_entry -Wl,--export-dynamic -Wl,--defsym,__ImageBase=0 --shared"])
	], [
	AC_SUBST(SGX_ENCLAVE_CFLAGS,
