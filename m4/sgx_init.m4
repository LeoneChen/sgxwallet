...
AC_DEFUN([SGX_INIT],[
	...
	AC_SUBST(SGX_ENCLAVE_CFLAGS,
		...
		-fsanitize-coverage=inline-8bit-counters,pc-table -fno-sanitize-link-runtime"])
	...
	AC_SUBST(SGX_ENCLAVE_LDADD,
		["-Wl,--no-undefined -Wl,--whole-archive -lSGXSanRTEnclave -l\$(SGX_TRTS_LIB) -Wl,--no-whole-archive -Wl,--start-group \$(SGX_EXTRA_TLIBS) -lsgx_tstdc -lsgx_tcrypto -lsgx_pthread -l\$(SGX_TSERVICE_LIB) -Wl,--end-group -Wl,-Bstatic -Wl,-Bsymbolic -Wl,-pie,-eenclave_entry -Wl,--export-dynamic -Wl,--defsym,__ImageBase=0"])
	], [
	AC_SUBST(SGX_ENCLAVE_CFLAGS,
	 	["-nostdinc -fvisibility=hidden -fpie -ffunction-sections -fdata-sections -fstack-protector"])
	AC_SUBST(SGX_ENCLAVE_CPPFLAGS, 
