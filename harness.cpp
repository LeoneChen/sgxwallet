...
static void harness_dkg_verify_v2_workflow(void) {
  ...
  // s_share: 192 chars expected (64 encr_sshare + 128 pubkey coords)
  size_t ss_len = g_fdp->ConsumeIntegralInRange<size_t>(64, 192);
  char *s_share = (char *)calloc(ss_len + 1, sizeof(char));
  g_alloc_mgr.push_back((uint8_t *)s_share);
  if (!s_share) return;
...
// Workflow 10: ECDSA multi-sign - trigger sigCounter % 1000 verification path
// Targets: trustedEcdsaSign line 561 (periodic verification every 1000 sigs)
static void harness_ecdsa_multi_sign(void) {
  int errStatus = 0;
  char *err_string = (char *)calloc(1024, sizeof(char));
  g_alloc_mgr.push_back((uint8_t *)err_string);
  if (!err_string) return;
  ...
  // Sign 1001+ times to trigger the sigCounter % 1000 == 0 verification path
  // Use a fixed hash to avoid consuming too much fuzz data per iteration
  char hash[65];
  fill_hex_string(hash, 64);
...
extern "C" void customized_init() {
  // ========================================================================
  // Step 1: Register V2-targeted harnesses (HIGHEST PRIORITY - uncovered paths)
  // ========================================================================

  // DkgVerifyV2 dedicated: 0/10 edges COMPLETELY UNCOVERED (weight 100)
  test_harness_registry[test_harness_count++] = {harness_dkg_verify_v2_workflow, 100};

  // CreateBlsKeyV2 dedicated: 2/19 edges, barely covered (weight 80)
  test_harness_registry[test_harness_count++] = {harness_create_bls_key_v2_workflow, 80};

  // GetEncryptedSecretShareV2 dedicated: 2/14 edges (weight 80)
  test_harness_registry[test_harness_count++] = {harness_get_encrypted_secret_share_v2_workflow, 80};

  // Full V2 roundtrip: GenSecret -> GetEncryptedSecretShareV2 -> DkgVerifyV2 (weight 60)
  test_harness_registry[test_harness_count++] = {harness_v2_full_roundtrip, 60};

  // ========================================================================
  // Step 2: Register new targeted harnesses for specific bottlenecks
  // ========================================================================

  // ECDSA multi-sign: triggers sigCounter % 1000 verification path (weight 30)
  test_harness_registry[test_harness_count++] = {harness_ecdsa_multi_sign, 30};

  // GetDecryptionShare with valid BLS key: 2/6 edges (weight 40)
  test_harness_registry[test_harness_count++] = {harness_decryption_share_workflow, 40};

  // EncryptKey variations: tests various key formats + DecryptKey paths (weight 30)
  test_harness_registry[test_harness_count++] = {harness_encrypt_key_variations, 30};

  // ========================================================================
  // Step 3: Register existing workflow harnesses (reduced weight - already covered)
  // ========================================================================

  // ECDSA workflow: GenKey -> GetPubKey/Sign/Decrypt (weight 20, was 50)
  test_harness_registry[test_harness_count++] = {harness_ecdsa_workflow, 20};

  // DKG workflow: GenDkgSecret -> Decrypt/GetPubShares/GetEncryptedSecretShare (weight 20, was 50)
  test_harness_registry[test_harness_count++] = {harness_dkg_workflow, 20};

  // BLS workflow: GenerateBLSKey -> Sign/GetPubKey (weight 20, was 50)
  test_harness_registry[test_harness_count++] = {harness_bls_workflow, 20};

  // CreateBlsKey V1 workflow: GenEcdsaKey -> CreateBlsKey (weight 10, was 40)
  test_harness_registry[test_harness_count++] = {harness_create_bls_key_workflow, 10};

  // DkgVerify V1 workflow: already 7 hits, 3/9 edges (weight 10, was 40)
  test_harness_registry[test_harness_count++] = {harness_dkg_verify_workflow, 10};

  // EncryptKey/DecryptKey V1 workflow (weight 10, was 30)
  test_harness_registry[test_harness_count++] = {harness_encrypt_decrypt_workflow, 10};

  // ========================================================================
  // Step 4: Register original harnesses at low weight (error path coverage)
  // ========================================================================

  // Generation functions (moderate weight for error paths)
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGenerateEcdsaKey, 3};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedEncryptKey, 3};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGenDkgSecret, 3};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGenerateBLSKey, 3};

  // Functions needing valid encrypted keys (low weight - test error paths)
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGetPublicEcdsaKey, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedEcdsaSign, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedDecryptKey, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedDecryptDkgSecret, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGetEncryptedSecretShare, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGetEncryptedSecretShareV2, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGetPublicShares, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedDkgVerify, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedDkgVerifyV2, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedCreateBlsKey, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedCreateBlsKeyV2, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedBlsSignMessage, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGetBlsPubKey, 2};
  test_harness_registry[test_harness_count++] = {
      _harness_trustedGetDecryptionShare, 2};
